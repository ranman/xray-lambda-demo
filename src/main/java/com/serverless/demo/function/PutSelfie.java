 ...
 */
public class PutSelfie implements RequestHandler<ServerlessInput, ServerlessOutput> {
    ...
    private static final String SELFIE_KEY = "key";
    // DynamoDB table attribute name for storing the bucket name that holds the article's content.
    private static final String SELFIE_BUCKET = System.getenv("SELFIE_BUCKET");
    ...
    public ServerlessOutput handleRequest(ServerlessInput serverlessInput, Context context) {
            ...
            try {
                ...
                attributes.put(SELFIE_KEY, new AttributeValue().withS(keyName));
                dynamoDb.putItem(new PutItemRequest()
                        .withTableName(DDB_TABLE)
                        .withItem(attributes));
                output.setStatusCode(200);
                output.setBody("Successfully inserted selfie " + attributes.get(SELFIE_KEY).getS());
            } catch (Exception e) {
